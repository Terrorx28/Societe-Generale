...
- The frontend visualizes anomaly detection results, risk scores, investigation insights, and evaluation metrics generated by the detection pipeline.
- The backend provides anomaly scoring, risk analysis, and AI-powered investigation services through a Flask API.
- For deployment, build frontend with `npm run build` and deploy `dist/` folder to any static host (GitHub Pages, Vercel, etc.)
- The backend can be deployed independently as a Flask service for real-time threat analysis and investigation workflows.
